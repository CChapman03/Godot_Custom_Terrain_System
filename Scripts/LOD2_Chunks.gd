...
func _ready():
	...
	# Create Chunk Mesh
	var chunk_mesh = preload("res://3D_ObJects/Terrain_Chunk_LOD2.obj") # : PlaneMesh = create_chunk_mesh()
	...
	var lod1_view_trapezoid : PackedVector3Array = Terrain.construct_trapezoid(Terrain.fov, Terrain.lod1_max_distance, Terrain.lod2_max_distance)
	
	# Calc instances to draw in multmesh (Based on inside view-frustum trapezoid)
	view_instances = calc_instances(lod1_view_trapezoid, $"../LOD1_Chunks".view_instances)
	...
	material.set_shader_parameter("lod_level", 2)
