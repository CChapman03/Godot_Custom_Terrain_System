...
func calc_instances(view_trapezoid : PackedVector3Array, lod0_instances):
	...
	for x in range(floor(minx_grid.x) - 1, ceil(maxx_grid.x) + 1):
		for z in range(floor(minz_grid.z) - 1, ceil(maxz_grid.z) + 1):
			...
			trans = trans.rotated(Vector3.UP, Terrain.player_head_transform.basis.y.normalized().z / deg_to_rad(90.0))
...
func _ready():
	...
	# Create Chunk Mesh
	var chunk_mesh = preload("res://3D_ObJects/terrain_chunk_lod0a.obj") #self.create_chunk_mesh()
	...
	var lod0a_view_trapezoid : PackedVector3Array = Terrain.construct_trapezoid(Terrain.fov, Terrain.lod0_max_distance, Terrain.lod0_max_distance + 50.0)
	
	# Calc instances to draw in multmesh (Based on inside view-frustum trapezoid)
	view_instances = calc_instances(lod0a_view_trapezoid, $"../LOD0_Chunks".view_instances)
...
func _process(delta):
	...
	if Engine.get_process_frames() % 4 == 0:
		# Constuct view-frustum trapezoid
		var lod1_view_trapezoid : PackedVector3Array = Terrain.construct_trapezoid(Terrain.fov, Terrain.lod0_max_distance, Terrain.lod0_max_distance + 50.0)
